...
async def main():
    ...
    log_info(
        f"📚 TravilyCrawl: Starting to crawl documentation from  https://python.langchain.com",
        Colors.PURPLE,
    )
...
#     Crawl the documentation site
    ...
    res = tavily_crawl.invoke({
        ...
        # "instructions":"content in ai agents"
    })
    ...
    log_success(f"Tavily Successfully crawled {len(all_docs)} URL's from documentation site.")
    log_info(f"✂️  Text Splitter: Processing  {len(all_docs)} documents with 4000 chubk size & 200 overlap.",Colors.YELLOW)
    ...
    log_success(f"✂️  Text Splitter: Created {len(splitted_docs)} chunks from {len(all_docs)} documents.")
